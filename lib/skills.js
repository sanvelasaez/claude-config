...
function installExternalSkill({ repo, name, flags, merge }) {
  const alreadyInstalled = isSkillInstalled(name);
  info(`${alreadyInstalled ? "Actualizando" : "Instalando"} skill externa: ${repo}`);
  ...
  if (r.status === 0 && isSkillInstalled(name)) {
    ok(`Skill externa '${name}' — ${alreadyInstalled ? "actualizada" : "instalada"}`);
  } else if (alreadyInstalled) {
    // npx skills add puede salir con error si ya está al día — se considera OK
    ok(`Skill externa '${name}' — ya al día`);
  } else {
