...
// Devuelve los nombres de archivo (sin ruta) referenciados en comandos de hooks
// de settings.json. Los plugins despliegan sus propios archivos en hooks/ y los
// registran en settings.json — esos archivos no son obsoletos aunque no estén en el repo.
function getActivePluginHookFiles() {
  try {
    ...
    for (const event of Object.values(s.hooks || {})) {
      for (const matcher of (Array.isArray(event) ? event : [])) {
        for (const hook of (matcher.hooks || [])) {
          ...
          for (const m of matches) names.add(path.basename(m));
        }
      }
    }
    return names;
...
function checkObsoleteFiles() {
  ...
  const activePluginHooks = getActivePluginHookFiles();
  const obsoletos = [];
  for (const item of COPY_ITEMS) {
    ...
    for (const f of getAllFilesRelative(claudeDst)) {
      ...
      if (item === "hooks" && activePluginHooks.has(path.basename(f))) continue;
...
module.exports = {
  ...
  getActivePluginHookFiles, checkObsoleteFiles,
};
