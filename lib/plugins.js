...
const { MARKETPLACES, PLUGINS, CLAUDE_DIR, readJson } = require("./config");
...
function verifyPluginInstalled(pluginId) {
  try {
    const pluginsJsonPath = path.join(CLAUDE_DIR, "plugins", "installed_plugins.json");
    const data = readJson(pluginsJsonPath);
...
function installPlugin({ name, marketplace }) {
  ...
  if (!verifyPluginInstalled(pluginId)) {
    warn(`Plugin '${name}' — installPath vacío tras instalación; reinstalando...`);
    spawnClaude(["plugin", "uninstall", pluginId], { stdio: "inherit" });
    const r2 = spawnClaude(["plugin", "install", pluginId], { stdio: "inherit" });
    if (r2.status !== 0) {
      warn(`Plugin '${name}' — reinstalación fallida; ejecutar manualmente: claude plugin install ${pluginId}`);
      return;
    }
  }
