...
function runChecks(pythonCmdHint) {
  ...
  console.log("  rtk");
  const rtkVer = checkRtk();
  if (rtkVer) ok(`rtk — ${rtkVer} (integración con centinel activa)`);
  else warn("rtk — no encontrado en PATH (puede requerir reiniciar el terminal)");
  sep();
