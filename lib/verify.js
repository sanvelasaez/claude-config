...
const { CLAUDE_DIR, CLAUDE_JSON, COPY_ITEMS, EXTERNAL_SKILLS, PLUGINS, readJson } = require("./config");
...
function runChecks(pythonCmdHint) {
  ...
  for (const { name, marketplace } of PLUGINS) {
    if (installedPlugins.includes(`${name}@${marketplace}`)) ok(`Plugin '${name}@${marketplace}'`);
    else {
...
module.exports = {
  checkFiles, checkSettingsJson, checkClaudeJson,
