...
function ensurePython3Alias() {
  if (process.platform !== "win32") return;
  const w = spawnSync("where", ["python"], { encoding: "utf8" });
  if (w.status !== 0) return;
  const pythonExe = w.stdout.trim().split(/\r?\n/)[0].trim();
  const python3Exe = path.join(path.dirname(pythonExe), "python3.exe");
  if (fs.existsSync(python3Exe)) {
    ok("python3 — alias ya existe");
    return;
  }
  try {
    fs.linkSync(pythonExe, python3Exe);
    ok("python3 — hardlink creado → python.exe");
  } catch (e) {
    warn(`No se pudo crear alias python3: ${e.message}`);
  }
}
...
module.exports = {
  ...
  ensureNode, ensureClaudeCode, ensureGit, ensurePython, ensurePython3Alias,
};
